...
首先代码逻辑上已经没什么问题的，但还需要优化一下，里面太多string int float 和数组之间的转化了，需要重新评估一下参数。其次没想到里面最难的是对小数的处理。尤其是四舍五入，此处小数位数是给定的，意味着不会越界，所以此处使用toFixed做四舍五入操作。同时针对进位的情况作了内部性能调优，减免因为进位+1导致的大整数加法的除法几率。未完待续。
