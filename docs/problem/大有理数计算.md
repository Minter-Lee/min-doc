...
> PS：这标题我改了，原来叫大有理数计算，写道后面总提小数的问题，不如就一并来了吧，查了半天，才知道包含有理数包含分数和整数。
...
写道这里，我总算确认了对负号进行补充的方法，利用类似facade的设计，在原有的API基础上，在提取一层函数作为最终API而，在这些API中，根据当前算法来处理符号问题，并根据结果调用相应的计算方法。那么原有的API将转为正数的计算，名称都会加入p后缀代表positive number。同时继续对外提供正数计算的接口，如果确认没有负数参与可直接使用。针对小数部分也如此。这里用加法举个例子，代码如下：
